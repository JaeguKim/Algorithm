...
void init(int tree[], int player[], int bin_num, int bin_capacity); //tree,flare 초기화
...
int main(void)
{
	...
	player = new int[bin_num + 1]; //flare 1~bin_num 까지 공간 할당
...
void insertToBin(int player[], int tree[], int bin_num, int &search_idx, int insert_num)
{
	while (1)
	{
		if (player[tree[search_idx]] >= insert_num) //현재 가리키는 위치에있는 bin의 용량이 insert_num보다 크거나 같으면
		{
			...
			{
				if (player[search_idx - bin_num + 1] >= insert_num) //bin의 용량이 insert_num 보다 크거나 같으면
					player[search_idx - bin_num + 1] = player[search_idx - bin_num + 1] - insert_num;
				else //bin 의 용량이 insert_num 보다 작으면
				{
					player[search_idx - bin_num + 2] = player[search_idx - bin_num + 2] - insert_num;
					search_idx+=1;
				}
				//tree를 업데이트
...
void update(int player[], int tree[], int bin_num, int &search_idx)
{
	while (search_idx != 1) //root노드에 도달할때까지 반복
	{
		if (search_idx % 2 == 0) //index번호가 짝수이면
		{
			...
			{
				if (player[search_idx - bin_num + 1] >= player[search_idx - bin_num + 2]) //왼쪽노드의 용량이 크거나 같으면
				{
		...
		else //index번호가 홀수이면
		{
			if (search_idx > bin_num - 1) //index번호가 tree배열 바운더리를 넘어서면
			{
				if (player[search_idx - bin_num] >= player[search_idx - bin_num + 1])
				{
			...
			{
				if (player[tree[search_idx - 1]] >= player[tree[search_idx]])
				{
					tree[search_idx / 2] = tree[search_idx - 1];
				}
				else
